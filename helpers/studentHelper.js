  ...
  markAttendance: (note, stdId, status) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (student.Attendance) {
        let topicExist = await student.Attendance.findIndex(
          (att) =>
            objectId(att.TopicId).toString() == objectId(note._id).toString()
        );
        console.log("topic", topicExist);
        if (topicExist != -1) {
          await student.Attendance.map(async (attendance) => {
            if (
              objectId(attendance.TopicId).toString() ==
              ...
              if (attendance.status) {
                ...
                await db
                  ...
                  .updateOne(
                    {
                      _id: objectId(stdId),
                      "Attendance.TopicId": objectId(note._id),
                    },
                    {
    ...
    return new Promise(async (resolve, reject) => {
      if (Data.type == "name") {
        let data = await db
          .get()
          .collection(collections.NOTES_COLLECTION)
          .find({ Name: { $regex: Data.val, $options: "$i" } })
          .toArray();
        try {
          resolve(data);
        } catch (error) {
        ...
        let data = await db
          .get()
          .collection(collections.NOTES_COLLECTION)
          .find({ Date: { $regex: newdate, $options: "$i" } })
          .toArray();
        try {
          resolve(data);
        } catch (error) {
