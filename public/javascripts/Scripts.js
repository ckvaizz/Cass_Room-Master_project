...
$(function () {
  $("#editstudentform").on("submit", function (event) {
    ...
    console.log("calling++");
    $(".studnteditconfirm1")
      .addClass("studnteditconfirm")
      .removeClass("studnteditconfirm1");
  });
});
...
deleteStudentconfrm = (Id) => {
  delteStudentId = Id;
  $(".deltestudentcnfrm1")
    .addClass("deltestudentcnfrm")
    .removeClass("deltestudentcnfrm1");
};
...
deltStudent = () => {
  $.ajax({
    ...
    success: () => {
      $(".deltestudentcnfrm")
        .addClass("deltestudentcnfrm1")
        .removeClass("deltestudentcnfrm");
      $("#tr" + delteStudentId).remove();
      delteStudentId = "";
      $(".studentremovedalert1")
        .addClass("studentremovedalert")
        .removeClass("studentremovedalert1");
    },
  });
};
removespan = () => {
  $(".studentremovedalert")
    .addClass("studentremovedalert1")
    .removeClass("studentremovedalert");
};
...
deleteAssignmentcnfrm = (Id) => {
  ...
  $(".deleteassignmentcnfrm1")
    .addClass("deleteassignmentcnfrm")
    .removeClass("deleteassignmentcnfrm1");
};
...
deleteAssignment = () => {
  $.ajax({
    ...
    method: "get",
    success: () => {
      $(".deleteassignmentcnfrm")
        .addClass("deleteassignmentcnfrm1")
        .removeClass("deleteassignmentcnfrm");
...
showoptions = () => {
  if (!option) {
    option = true;
    $(".passwordoptions1")
      .addClass("passwordoptions")
      .removeClass("passwordoptions1");
  } else {
    option = false;
    $(".passwordoptions")
      .addClass("passwordoptions1")
      .removeClass("passwordoptions");
  }
};
creatPassWordForm = () => {
  $(".stdLoginForm").addClass("stdLoginForm1").removeClass("stdLoginForm");
  $(".stdCreatePassword1")
    .addClass("stdCreatePassword")
    .removeClass("stdCreatePassword1");
};

backToLogin = () => {
  $(".stdLoginForm1").addClass("stdLoginForm").removeClass("stdLoginForm1");
  $(".stdCreatePassword")
    .addClass("stdCreatePassword1")
    .removeClass("stdCreatePassword");
};
let stdMobile = "";
  ...
  $.ajax({
    ...
    success: (response) => {
      if (response.status === true) {
        ...
        $(".stdNewpassword1")
          .addClass("stdNewpassword")
          .removeClass("stdNewpassword1");
      } else {
    ...
    $.ajax({
      ...
      success: (response) => {
        if (response.status) {
          ...
          $(".otp-mob-err1")
            .addClass("otp-mob-err")
            .removeClass("otp-mob-err1");
        }
      },
    });
  } else {
...
$("#Searchnote").on("change keyup paste", () => {
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      if (data != "") {
        document.getElementById("searchNote").innerHTML = `${data.map(
          (note) => {
...
changeSearch = () => {
  if (searchType == "name") {
    ...
    $(".searchwithntext1")
      .addClass("searchwithntext")
      .removeClass("searchwithntext1");
    $(".searchwithdText")
      .addClass("searchwithdText1")
      .removeClass("searchwithdText");
    searchType = "date";
  } else {
    $(".searchName1").addClass("searchName").removeClass("searchName1");
    $(".searchDate").addClass("searchDate1").removeClass("searchDate");
    $(".searchwithntext")
      .addClass("searchwithntext1")
      .removeClass("searchwithntext");
    $(".searchwithdText1")
      .addClass("searchwithdText")
      .removeClass("searchwithdText1");
    ...
    $(this).ajaxSubmit({
      ...
      success: function (response) {
        setTimeout(() => {
          ...
          $(".uploadedstatus1")
            .addClass("uploadedstatus")
            .removeClass("uploadedstatus1");
          $("#assFile").val(" ");
          setTimeout(() => {
            $(".uploadedstatus")
              .addClass("uploadedstatus1")
              .removeClass("uploadedstatus");
          }, 2000);
        }, 2000);
      },
    });
    ...
    success: (data) => {
      ...
      if (data != " ") {
        document.getElementById("back" + Id).innerHTML = `
            ...
            <div class="datadetails"><h5><a href="/view-Assignment?id=${
              data.TopicId
            }">${data.Topic}</a></h5>
            <p>Date : ${data.Date}</p>
            <p>Submited Time : ${data.Time}</p>
            <p>Mark : ${data.Mark ? data.Mark : `Not valued`} </p>
            <a href="/viewsubmited?id=${
              data.TopicId
            }" class="btn btn-primary"> view submited </a> 
            </div>
            `;
      } else {
        document.getElementById("back" + Id).innerHTML = `
...
function openOptions_ann(id) {
  ...
  $("#btn" + id)
    .addClass("optionbtnAnn1")
    .removeClass("optionbtnAnn");
  $("#btnc" + id)
    .addClass("optioncnlAnn")
    .removeClass("optioncnlAnn1");
}
function cancelOption_ann(id) {
  let div = document.querySelector("#frnt" + id);
  div.style = "transform:translate(0px)";
  $("#btn" + id)
    .addClass("optionbtnAnn")
    .removeClass("optionbtnAnn1");
  $("#btnc" + id)
    .addClass("optioncnlAnn1")
    .removeClass("optioncnlAnn");
}
var delteAnnId = "";
...
function showAttstatus() {
  ...
  $(".todayTaskstatus1")
    .addClass("todayTaskstatus")
    .removeClass("todayTaskstatus1");
}
...
function razorPayment(data) {
  var options = {
    ...
    image:
      "https://lh3.googleusercontent.com/proxy/yqFfR8_Li7B6SorbTcnd_lCYrrh5i0oVYsdqRdUkxfIT8Dm0WHxie8SObIvM8iwVlV3TW8TAQ2-ebFsHcP3jd9TPZqInMPzrBN5P-WC_hHP_W-NNAAUaDzwwEQdpAurLTdEmCIPPb7thkbxilYAd5v3vnMJwYJNcLsgGR_sGt26Er-0owIKCs_SF8tSSpwFlLPWsXwIMPxJYiWkpWt0Nm84SDbFEVZiY_g",
    order_id: data.id,
...
function openhomeevnt() {
  if (!openevnt) {
    $(".homeevnt-cntent1")
      .addClass("homeevnt-cntent")
      .removeClass("homeevnt-cntent1");
    openevnt = true;
  } else {
    $(".homeevnt-cntent")
      .addClass("homeevnt-cntent1")
      .removeClass("homeevnt-cntent");
    openevnt = false;
  }
}
var feeoption = false;
...
function show_stdmsg(id) {
  ...
  $(".studentchat-message1")
    .addClass("studentchat-message")
    .removeClass("studentchat-message1");
  $.ajax({
    ...
    success: (data) => {
      ...
      if (data) {
        ...
        if (data.Messages) {
          document.getElementById("msg-body").innerHTML = data.Messages.map(
            (msg) => {
              return `
...
function asssearch() {
  let searchVal = $("#assSearch").val();
  if (searchVal.length != 0) {
    $(".asssearch-div1")
      .addClass("asssearch-div")
      .removeClass("asssearch-div1");
    $.ajax({
      url: "/assSearch",
      method: "post",
      data: {
      ...
      success: (data) => {
        if (!data.Error) {
          if (data != "") {
            $(".searchLoad").addClass("searchLoad1").removeClass("searchLoad");
            document.getElementById("asssearch-div").innerHTML = `
                        ${data.map((val) => {
                          return `
                            
                            <div class="col-md-3" >
                <div class="ass-card">
                <div id="div${val._id}" class="ass-cardInner">
                   <div class="front-ass">
                        <h5>${val.Name}</h5>
                    <p>Date : ${val.Date}</p>
                    <button onclick="view_assdetails('${val._id}')" class="btn btn-primary">More details</button>
                   </div>
                   <div id="back${val._id}" class="back-ass">
                       <h1 class="backload"><i class="fa fa-spinner fa-spin"></i></h1>
            ...
            $(".searchLoad").addClass("searchLoad1").removeClass("searchLoad");
            document.getElementById("asssearch-div").innerHTML = `
                        <h4 style="margin:auto;">No result</h4>
                        `;
          }
        } else {
          alert("koii");
    ...
    $(".asssearch-div").addClass("asssearch-div1").removeClass("asssearch-div");
  }
}

function closeasssearch() {
  $("#assSearch").val("");
  $(".asssearch-div").addClass("asssearch-div1").removeClass("asssearch-div");
}
