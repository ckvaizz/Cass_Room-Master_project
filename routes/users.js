...
paypal.configure({
  ...
  client_id:
    "AZxmIss0kurFWl7jACnwAocWcWAJ5ruCJK-9VEjGmDgBhTyvknVYnMajGycuJwPdbDRKmzNUDitwAwDK",
  client_secret:
    "EOv-WdYNe88YyoXKTKjqgpXM4HsbtcooXimj1BNDQdCGuYpzr4XChUiLUyBgUpngK9EdgnncbCk96trD",
});
...
router.post("/verifyOtp", (req, res) => {
  studentHelper
    .checkOtp(req.body, req.query.No)
    .then((response) => {
      res.json(response);
    })
    .catch((response) => {
...
router.get("/loginstd-otp", async (req, res) => {
  ...
  studentHelper.getStudent(req.query.num).then((student) => {
    ...
    res.render("students/std-Home", {
      Messages,
      events,
      todayStatus,
      student,
      studentLogin: req.session.studentLogin,
      login: true,
      announcements,
    });
  });
...
router.get("/logintrue", verifyLogin, async (req, res) => {
  ...
  if (req.session.studentLogin) {
    Socket.getVideoLink().then((link) => {
      res.render("students/std-Home", {
        VideocallLink: link.status ? link.Link : false,
        Messages,
        events,
...
router.get("/todayTask", verifyLogin, async (req, res) => {
  ...
  try {
    ...
    res.render("students/std-todayTask", {
      studentLogin: req.session.studentLogin,
      login: true,
      student: req.session.student,
      assignment,
...
router.post("/submitAssignment", verifyLogin, (req, res) => {
  ...
  studentHelper
    .submitAssignment(file, req.session.student._id)
    .then((respose) => {
      let pdf = req.files.File;
      pdf.mv(
        `./public/datas/assignments/submited/${
          req.body.assId + req.session.student._id
        }.pdf`
      );
      res.json("okke");
    });
...
router.get("/assignments", verifyLogin, async (req, res) => {
  let assignments = await studentHelper.getAssignments();
  try {
    res.render("students/std-assignments", {
      assignments,
      login: true,
      student: req.session.student,
      studentLogin: req.session.studentLogin,
...
router.get("/getAssignment", verifyLogin, async (req, res) => {
  let student = await studentHelper.getStudentDetails(req.session.student._id);
  let assnment = " ";
  await student.Assignments.map((ass) => {
    if (ass.TopicId == req.query.id) {
      assnment = ass;
    }
  });
  res.json(assnment);
...
router.get("/viewsubmited", verifyLogin, (req, res) => {
  let path = `./public/datas/assignments/submited/${
    req.query.id + req.session.student._id
  }.pdf`;
  var file = fs.createReadStream(path);
  var stat = fs.statSync(path);
  res.setHeader("Content-Length", stat.size);
...
router.get("/gallery", verifyLogin, async (req, res) => {
  ...
  try {
    res.render("students/std-gallery", {
      login: true,
      student: req.session.student,
      Photos,
    });
...
router.get("/attendance", verifyLogin, async (req, res) => {
  ...
  res.render("students/std-attendance", {
    login: true,
    student: req.session.student,
    stdAttedance,
  });
...
router.get("/attendanceM", verifyLogin, async (req, res) => {
  ...
  console.log("++", month);
  let totalDate = new Date(month[1], month[0], 0).getDate();
  let count = 0;
  let stdAttedance = [];
  Attendance.map((att) => {
  ...
  let percentage = (count / totalDate) * 100;
...
router.get("/announcements", verifyLogin, async (req, res) => {
  let announcements = await studentHelper.getAnnouncements();
  res.render("students/std-announcements", {
    login: true,
    student: req.session.student,
    announcements,
  });
...
router.get("/view-announcement", verifyLogin, async (req, res) => {
  ...
  res.render("students/std-viewannouncement", {
    login: true,
    student: req.session.student,
    announcement,
  });
...
router.get("/events", verifyLogin, async (req, res) => {
  ...
  res.render("students/std-events", {
    paidEvents,
    login: true,
    student: req.session.student,
    events,
...
router.post("/verify-payment-Razor", verifyLogin, (req, res) => {
  studentHelper
    .verifyPayment_Razor(req.body)
    .then(async (response) => {
      await studentHelper
        .addPayedstatus(req.session.student, payedEventId)
        .then((r) => {
          payedEventId = "";
...
router.get("/paypalErr", (req, res) => {
  ...
  payedEventId = "";
  res.send(
    '<center>Payment Error Try again..</center></br> <a href="/events">Go Back To Events</a>'
  );
});
...
router.get("/payEventPaytm", verifyLogin, (req, res) => {
  ...
  check_sum.genchecksum(params, key, function (err, checksum) {
    ...
    res.write(
      '<center><h1>Please do not refresh this page...</h1></center><form method="post" action="' +
        txn_url +
        '" name="f1">' +
        form_fields +
...
router.post("/paytmresJponse", async (req, res) => {
  let data = req.body;
  if (data.STATUS == "TXN_SUCCESS") {
    ...
    res.send(
      "<center><h2>Payment Failled</h2></br> " +
        data.RESPMSG +
        '</br><a href="/events">Go Back..</a></center>'
    );
  }
});
...
router.get("/view-event", verifyLogin, async (req, res) => {
  let event = await studentHelper.getEvent(req.query.id);
  res.render("students/std-viewEvents", {
    event,
    login: true,
    student: req.session.student,
  });
...
router.post("/paytmFEEresponse", async (req, res) => {
  ...
  if (data.STATUS == "TXN_SUCCESS") {
    ...
    res.send(
      "<center><h2>Payment Failled</h2></br> " +
        data.RESPMSG +
        '</br><a href="/login">Go Back..</a></center>'
    );
  }
});
...
router.post("/sendMessage", verifyLogin, (req, res) => {
  ...
  studentHelper
    .sendMessage(msg, req.session.student._id)
    .then((data) => {
...
router.post("/assSearch", verifyLogin, (req, res) => {
  studentHelper
    .searchAss(req.body.value)
